...
#if defined (hiir_test_NEON) && hiir_WORD_SIZE == 64
#include "hiir/Downsampler2x2F64Neon.h"
#include "hiir/Downsampler2xF64Neon.h"
#include "hiir/HalfBand2F64Neon.h"
#include "hiir/HalfBandF64Neon.h"
#include "hiir/PhaseHalfPi2F64Neon.h"
#include "hiir/PhaseHalfPiF64Neon.h"
#include "hiir/Upsampler2x2F64Neon.h"
#include "hiir/Upsampler2xF64Neon.h"
#endif
...
void	TestAllClassesSpeed <NC>::perform_test () noexcept
{
	...
	if (cpu._neon_flag)
	{
		typedef	SpeedTester <AuxProc11 <HalfBandF64Neon <NBR_COEFS> > > TestType;
		AlignedObject <TestType>   container;
		TestType &     st = container.use ();
		st.perform_test ("HalfBandF64Neon", "process_block");
	}
	if (cpu._neon_flag)
	{
		typedef	SpeedTester <AuxProc11 <HalfBand2F64Neon <NBR_COEFS> > > TestType;
		AlignedObject <TestType>   container;
		TestType &     st = container.use ();
		st.perform_test ("HalfBand2F64Neon", "process_block");
	}
#endif
	...
	if (cpu._neon_flag)
	{
		typedef	SpeedTester <AuxProc12Split <HalfBandF64Neon <NBR_COEFS> > > TestType;
		AlignedObject <TestType>   container;
		TestType &     st = container.use ();
		st.perform_test ("HalfBandF64Neon", "process_block_split");
	}
	if (cpu._neon_flag)
	{
		typedef	SpeedTester <AuxProc12Split <HalfBand2F64Neon <NBR_COEFS> > > TestType;
		AlignedObject <TestType>   container;
		TestType &     st = container.use ();
		st.perform_test ("HalfBand2F64Neon", "process_block_split");
	}
#endif
	...
	if (cpu._neon_flag)
	{
		typedef	SpeedTester <AuxProc11 <Downsampler2xF64Neon <NBR_COEFS> > > TestType;
		AlignedObject <TestType>   container;
		TestType &     st = container.use ();
		st.perform_test ("Downsampler2xF64Neon", "process_block");
	}
	if (cpu._neon_flag)
	{
		typedef	SpeedTester <AuxProc11 <Downsampler2x2F64Neon <NBR_COEFS> > > TestType;
		AlignedObject <TestType>   container;
		TestType &     st = container.use ();
		st.perform_test ("Downsampler2x2F64Neon", "process_block");
	}
#endif
	...
	if (cpu._neon_flag)
	{
		typedef	SpeedTester <AuxProc12Split <Downsampler2xF64Neon <NBR_COEFS> > > TestType;
		AlignedObject <TestType>   container;
		TestType &     st = container.use ();
		st.perform_test ("Downsampler2xF64Neon", "process_block_split");
	}
	if (cpu._neon_flag)
	{
		typedef	SpeedTester <AuxProc12Split <Downsampler2x2F64Neon <NBR_COEFS> > > TestType;
		AlignedObject <TestType>   container;
		TestType &     st = container.use ();
		st.perform_test ("Downsampler2x2F64Neon", "process_block_split");
	}
#endif
	...
	if (cpu._neon_flag)
	{
		typedef	SpeedTester <AuxProc11 <Upsampler2xF64Neon <NBR_COEFS> > > TestType;
		AlignedObject <TestType>   container;
		TestType &     st = container.use ();
		st.perform_test ("Upsampler2xF64Neon", "process_block");
	}
	if (cpu._neon_flag)
	{
		typedef	SpeedTester <AuxProc11 <Upsampler2x2F64Neon <NBR_COEFS> > > TestType;
		AlignedObject <TestType>   container;
		TestType &     st = container.use ();
		st.perform_test ("Upsampler2x2F64Neon", "process_block");
	}
#endif
	...
	if (cpu._neon_flag)
	{
		typedef	SpeedTester <AuxProc12 <PhaseHalfPiF64Neon <NBR_COEFS> > > TestType;
		AlignedObject <TestType>   container;
		TestType &     st = container.use ();
		st.perform_test ("PhaseHalfPiF64Neon", "process_block");
	}
	if (cpu._neon_flag)
	{
		typedef	SpeedTester <AuxProc12 <PhaseHalfPi2F64Neon <NBR_COEFS> > > TestType;
		AlignedObject <TestType>   container;
		TestType &     st = container.use ();
		st.perform_test ("PhaseHalfPi2F64Neon", "process_block");
	}
#endif
