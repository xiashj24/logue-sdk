...
#pragma once
#if ! defined (hiir_PhaseHalfPiF64Neon_CODEHEADER_INCLUDED)
#define hiir_PhaseHalfPiF64Neon_CODEHEADER_INCLUDED
...
template <int NC>
PhaseHalfPiF64Neon <NC>::PhaseHalfPiF64Neon () noexcept
:	_bifilter ()
,	_prev (0)
,	_phase (0)
{
...
template <int NC>
void	PhaseHalfPiF64Neon <NC>::set_coefs (const double coef_arr []) noexcept
{
...
template <int NC>
void	PhaseHalfPiF64Neon <NC>::process_sample (double &out_0, double &out_1, double input) noexcept
{
	auto           x = vcombine_f64 (vdup_n_f64 (_prev), vdup_n_f64 (input));
	StageProcF64Neon <_nbr_stages>::process_sample_neg (
...
template <int NC>
void	PhaseHalfPiF64Neon <NC>::process_block (double out_0_ptr [], double out_1_ptr [], const double in_ptr [], long nbr_spl) noexcept
{
	...
	while (pos < end)
	{
		...
		auto           x_0     = vextq_f64 (prev, input_0, 1); // prev [1], input_0 [0]
		StageProcF64Neon <_nbr_stages>::process_sample_neg (
			x_0, &_bifilter [0] [0]
		);

		const auto     input_1 = vdupq_n_f64 (in_ptr [pos + 1]);
		auto           x_1     = vextq_f64 (input_0, input_1, 1); // prev = input_0
		StageProcF64Neon <_nbr_stages>::process_sample_neg (
			x_1, &_bifilter [1] [0]
		);

		const auto     y_0 = vzip2q_f64 (x_0, x_1);
		const auto     y_1 = vzip1q_f64 (x_0, x_1);
		storeu (out_0_ptr + pos, y_0);
		storeu (out_1_ptr + pos, y_1);
...
template <int NC>
void	PhaseHalfPiF64Neon <NC>::clear_buffers () noexcept
{
	for (int phase = 0; phase < _nbr_phases; ++phase)
	{
		for (int i = 0; i < _nbr_stages + 1; ++i)
...
#endif // hiir_PhaseHalfPiF64Neon_CODEHEADER_INCLUDED
