...
#pragma once
#if ! defined (hiir_HalfBandF64Neon_CODEHEADER_INCLUDED)
#define hiir_HalfBandF64Neon_CODEHEADER_INCLUDED
...
template <int NC>
HalfBandF64Neon <NC>::HalfBandF64Neon () noexcept
:	_bifilter ()
,	_prev (0)
,	_phase (0)
{
	for (int phase = 0; phase < _nbr_phases; ++phase)
	{
		...
		{
		   storea (_bifilter [phase] [i]._coef, vdupq_n_f64 (0));
		}
...
template <int NC>
void	HalfBandF64Neon <NC>::set_coefs (const double coef_arr []) noexcept
{
...
template <int NC>
double	HalfBandF64Neon <NC>::process_sample (double input) noexcept
{
...
template <int NC>
void	HalfBandF64Neon <NC>::process_block (double out_ptr [], const double in_ptr [], long nbr_spl) noexcept
{
...
template <int NC>
double	HalfBandF64Neon <NC>::process_sample_hpf (double input) noexcept
{
	const auto     y    = process_2_paths (input);
...
template <int NC>
void	HalfBandF64Neon <NC>::process_block_hpf (double out_ptr [], const double in_ptr [], long nbr_spl) noexcept
{
	assert (out_ptr != nullptr);
	assert (out_ptr <= in_ptr || out_ptr >= in_ptr + nbr_spl * _nbr_chn);
...
template <int NC>
void	HalfBandF64Neon <NC>::process_sample_split (double &low, double &high, double input) noexcept
{
	const auto     y = process_2_paths (input);
	low  = y [0] + y [1];
	high = y [0] - y [1];
...
template <int NC>
void	HalfBandF64Neon <NC>::process_block_split (double out_l_ptr [], double out_h_ptr [], const double in_ptr [], long nbr_spl) noexcept
{
...
template <int NC>
void	HalfBandF64Neon <NC>::clear_buffers () noexcept
{
	for (int phase = 0; phase < _nbr_phases; ++phase)
	{
		for (int i = 0; i < _nbr_stages + 1; ++i)
		{
			storea (_bifilter [phase] [i]._mem, vdupq_n_f64 (0));
		}
	}
...
template <int NC>
std::array <double, 2>	HalfBandF64Neon <NC>::process_2_paths (double input) noexcept
{
	auto           x    = vcombine_f64 (vdup_n_f64 (_prev), vdup_n_f64 (input));
	StageProcF64Neon <_nbr_stages>::process_sample_pos (
		x, &_bifilter [_phase] [0]
	);
	const auto     half = vdupq_n_f64 (0.5);
	x *= half;
	const auto     even = vgetq_lane_f64 (x, 1);
	const auto     odd  = vgetq_lane_f64 (x, 0);
...
void	HalfBandF64Neon <NC>::process_block_2_paths (double out_l_ptr [], double out_h_ptr [], const double in_ptr [], long nbr_spl, FL fnc_l, FH fnc_h) noexcept
{
	...
	auto           prev = vdupq_n_f64 (_prev);
	while (pos < end)
	{
		const auto     input_0 = vdupq_n_f64 (in_ptr [pos    ]);
		const auto     input_1 = vdupq_n_f64 (in_ptr [pos + 1]);

		auto           x_0 = vzip1q_f64 (prev, input_0);
		auto           x_1 = vzip1q_f64 (input_0, input_1); // prev = input_0
		...
		const auto     even_0 = vgetq_lane_f64 (x_0, 1);
		const auto     odd_0  = vgetq_lane_f64 (x_0, 0);
		const auto     even_1 = vgetq_lane_f64 (x_1, 1);
		const auto     odd_1  = vgetq_lane_f64 (x_1, 0);
...
template <int NC>
void	HalfBandF64Neon <NC>::store_low (double *ptr, double even, double odd) noexcept
{
	*ptr = even + odd;
}
...
template <int NC>
void	HalfBandF64Neon <NC>::store_high (double *ptr, double even, double odd) noexcept
{
	*ptr = even - odd;
}
...
#endif // hiir_HalfBandF64Neon_CODEHEADER_INCLUDED
