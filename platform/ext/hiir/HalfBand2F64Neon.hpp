...
#pragma once
#if ! defined (hiir_HalfBand2F64Neon_CODEHEADER_INCLUDED)
#define hiir_HalfBand2F64Neon_CODEHEADER_INCLUDED
...
template <int NC>
HalfBand2F64Neon <NC>::HalfBand2F64Neon () noexcept
:	_bifilter ()
,	_phase (0)
{
	for (int i = 0; i < NBR_COEFS + 2; ++i)
	{
		storea (_bifilter [0] [i]._coef, vdupq_n_f64 (0));
		storea (_bifilter [1] [i]._coef, vdupq_n_f64 (0));
	}
...
template <int NC>
void	HalfBand2F64Neon <NC>::set_coefs (const double coef_arr []) noexcept
{
	assert (coef_arr != nullptr);

	for (int i = 0; i < NBR_COEFS; ++i)
	{
		const auto     c = vdupq_n_f64 (DataType (coef_arr [i]));
...
template <int NC>
float64x2_t	HalfBand2F64Neon <NC>::process_sample (float64x2_t input) noexcept
{
...
template <int NC>
void	HalfBand2F64Neon <NC>::process_block (double out_ptr [], const double in_ptr [], long nbr_spl) noexcept
{
...
template <int NC>
float64x2_t	HalfBand2F64Neon <NC>::process_sample_hpf (float64x2_t input) noexcept
{
	const auto     path_arr = process_2_paths (input);
...
template <int NC>
void	HalfBand2F64Neon <NC>::process_block_hpf (double out_ptr [], const double in_ptr [], long nbr_spl) noexcept
{
	assert (out_ptr != nullptr);
	assert (out_ptr <= in_ptr || out_ptr >= in_ptr + nbr_spl * _nbr_chn);
...
template <int NC>
void	HalfBand2F64Neon <NC>::process_sample_split (float64x2_t &low, float64x2_t &high, float64x2_t input) noexcept
{
	const auto     path_arr = process_2_paths (input);
...
template <int NC>
void	HalfBand2F64Neon <NC>::process_block_split (double out_l_ptr [], double out_h_ptr [], const double in_ptr [], long nbr_spl) noexcept
{
...
template <int NC>
void	HalfBand2F64Neon <NC>::clear_buffers () noexcept
{
	_phase = 0;
	for (int i = 0; i < NBR_COEFS + 2; ++i)
	{
		storea (_bifilter [0] [i]._mem, vdupq_n_f64 (0));
		storea (_bifilter [1] [i]._mem, vdupq_n_f64 (0));
	}
	storea (_prev, vdupq_n_f64 (0));
}
...
template <int NC>
typename HalfBand2F64Neon <NC>::EvenOdd	HalfBand2F64Neon <NC>::process_2_paths (float64x2_t input) noexcept
{
	const auto     half = vdupq_n_f64 (0.5f);
	input *= half;

	auto           path_arr = EvenOdd { input, load2f64a (_prev) };
...
void	HalfBand2F64Neon <NC>::process_block_2_paths (double out_l_ptr [], double out_h_ptr [], const double in_ptr [], long nbr_spl, FL fnc_l, FH fnc_h) noexcept
{
	...
	if (_phase == 1)
	{
		const auto     path_arr = process_2_paths (load2f64u (in_ptr));
	...
	auto           prev = load2f64a (_prev);
	while (pos < end)
	{
		const auto     ofs_0   = pos * _nbr_chn;
		const auto     ofs_1   = ofs_0 + _nbr_chn;
		auto           input_0 = load2f64u (in_ptr + ofs_0);
		auto           input_1 = load2f64u (in_ptr + ofs_1);
	...
	if (pos < nbr_spl)
	{
		...
		const auto     path_arr = process_2_paths (load2f64u (in_ptr + ofs));
...
template <int NC>
void	HalfBand2F64Neon <NC>::store_low (double *ptr, float64x2_t tmp_0, float64x2_t tmp_1) noexcept
{
...
template <int NC>
void	HalfBand2F64Neon <NC>::store_high (double *ptr, float64x2_t tmp_0, float64x2_t tmp_1) noexcept
{
...
#endif // hiir_HalfBand2F64Neon_CODEHEADER_INCLUDED
